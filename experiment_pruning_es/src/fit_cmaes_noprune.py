"""CMA-ES + Optuna for seven-parameter plate fitting — NO-PRUNING variant.

This is a fork of `src/cmaes/fit_7param_norm_es.py` with one change:
the Optuna pruner is `NopPruner()` instead of `SuccessiveHalvingPruner(15, 3)`.
Every CMA-ES trial runs to its full per-trial eval budget
(or until CMA-ES's internal `tolfun`/`tolx` triggers, or until the
study-level `--early_stop_loss` callback fires).

Run as:
    python -m experiment_pruning_es.src.fit_cmaes_noprune \\
...
Imports plate model + loss selector from the existing `src/` package — i.e.
this file does not duplicate those, only the orchestration code.
"""
...
# Existing repo modules — NOT duplicated in this folder.
...
def create_robust_storage(db_name: str = "cmaes_noprune.db"):
    import sqlite3
...
def physical_to_plate14_tensor(phys_batch_np: np.ndarray, device) -> torch.Tensor:
    E = phys_batch_np[:, 0]
    rho = phys_batch_np[:, 1]
    h = phys_batch_np[:, 2]
    Ly = phys_batch_np[:, 3]
...
def run_cmaes(
    ...
    """CMA-ES inner loop. Identical to fit_7param_norm_es.run_cmaes; the only
    difference between this experiment and the parent is the pruner config in
    the OUTER study, not anything inside this function. The trial.report /
    trial.should_prune calls remain — with NopPruner active, should_prune()
    always returns False and report() persists per-gen best loss into the
    Optuna RDBStorage as free observability."""
    ...
    while not es.stop():
        ...
        try:
            ...
            if trial.should_prune():
                # With NopPruner this branch is unreachable, but keep the
                # structure intact for direct comparability with the SHP run.
    ...
    print(f"        [Done] Trial {trial.number} best={es.result.fbest:.4f} evals={eval_count}")
    return best_p, es.result.fbest, loss_history, eval_count
...
def plot_per_ir(stem, est_7, gt_7, loss_hist, elapsed, out_path, gt_loss=None):
    ...
    ax.set_xlabel("Eval")
    ax.set_ylabel("Loss")
    ax.set_title("Optimization Convergence (winning trial)")
    ax.legend()
...
def run(args):
    ...
    print("=" * 70)
    print(f"CMA-ES (NO PRUNING) + LHS + Normalized Space on {device.upper()}")
    print("=" * 70)
    print(f"  Loss: {args.loss}")
    print(f"  Available losses: {available}")
    print(f"  Trials: {args.n_trials}")
    print(f"  Per-trial budget: {args.budget}")
    print(f"  Duration: {args.duration}s")
    print(f"  sigma0: {args.sigma0}")
    print(f"  Pruner: NopPruner  (every trial runs to completion)")
    ...
    for idx, npz_file in enumerate(npz_files):
        ...
        study = optuna.create_study(
            study_name=f"noprune_{npz_file.stem}",
            storage=storage,
            load_if_exists=True,
            direction="minimize",
            # >>> KEY DIFFERENCE FROM fit_7param_norm_es.py <<<
        ...
        def objective(trial):
            ...
            trial.set_user_attr("evals_used", int(evals_used))
        ...
        total_evals_used = int(
            sum(int(t.user_attrs.get("evals_used", 0)) for t in trials if t.state == optuna.trial.TrialState.COMPLETE)
        )
        ...
        # Per-trial flat CSV (cheap analytics without cracking the Optuna DB).
        ...
        plot_per_ir(npz_file.stem, best_params, gt_7, best_loss_hist, elapsed, output_path, gt_loss)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output_dir", type=str, default="results/experiment_pruning_es/cmaes_noprune_5k")
    ...
    parser.add_argument(
        "--storage",
        type=str,
        default="results/experiment_pruning_es/cmaes_noprune_5k/cmaes_noprune.db",
    )
