...
def main() -> None:
    ...
    p.add_argument("--hop", nargs="+", default=None, metavar="SET",
                   help="One comma-joined set per --n-fft set, same shape. "
                        ...
                        "the losses use.")
    p.add_argument("--eps", type=float, default=None, metavar="E",
                   help="ONE ABSOLUTE floor on every rung, which is what the "
                        "loss does: _make_stft_l1 takes a single eps and "
                        "applies it to each resolution's magnitudes unchanged. "
                        "--eps 1e-2 is the L1_STFT_eps1e2 family. This is NOT "
                        "the same experiment as --floor-db, and the difference "
                        "is the point: torch.stft is unnormalized, so a "
                        "peak-1 signal's bins scale with the window length, and "
                        "a fixed eps therefore sits ~12-24 dB deeper at 8192 "
                        "than at 512 across the _m5 ladder. The short rungs of "
                        "an eps arm are consequently far closer to linear than "
                        "the long ones -- a hybrid by the back door, which "
                        "--floor-db normalizes away by construction. Mutually "
                        "exclusive with --floor-db.")
    p.add_argument("--norm", default="target", choices=("target", "self", "none"),
                   help="How the two signals are put on a common scale before "
                        "the STFT. target (default) divides BOTH by the "
                        "TARGET's peak, which is exactly peak_normalized(mode="
                        "'target') in train_encoder and therefore what the "
                        "plate trains with. self divides each by its own peak "
                        "-- what this tool used to do unconditionally, and what "
                        "reproduces tables run before this flag existed. It is "
                        "not a neutral choice: self deletes the level "
                        "difference between candidate and target, and level is "
                        "the only place mu*Ly is visible, so it removes the cue "
                        "the linear term reads best. none leaves the renders "
                        "alone. For a homogeneous loss target and none give the "
                        "same concordance; they differ for a log loss, because "
                        "a fixed eps does not scale with the signal.")
    ...
    # Where the floor actually lands. Printed rather than assumed: the whole
    # question of whether an eps arm is really a log arm is the question of
    # where eps sits in the bin distribution, and that is not knowable from
    # the eps value alone -- it depends on the reference peak, which depends
    # on the window length and on --norm.
    scale: dict = {}
