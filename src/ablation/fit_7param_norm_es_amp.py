...
Run as:
    python -m src.ablation.fit_7param_norm_es_amp --loss CQT+LogDec ...
"""
...
def load_target_ir_from_npz(npz_path: Path, duration_s: float, expected_sr: int) -> np.ndarray:
    """Load unnormalized IR target from dataset NPZ."""
...
def run_cmaes(
    ...
    while not es.stop():
        ...
        with torch.no_grad():
            audios = synth(plate_params, duration, normalize=False)
        ...
        try:
            ...
            if trial.should_prune():
                print(f"        [Pruned] Trial {trial.number} at gen {gen} (loss={current_best:.4e})")
        ...
        if gen % 20 == 0:
            print(f"        [Trial {trial.number}] gen {gen:4d}  evals={eval_count:5d}  best={current_best:.4e}")
    ...
    print(f"        [Done] Trial {trial.number} best={es.result.fbest:.4e}")
...
def plot_per_ir(stem, est_7, gt_7, loss_hist, best_loss, elapsed, out_path, gt_loss=None):
    ...
    if loss_hist:
        ...
        if gt_loss is not None:
            ax.axhline(gt_loss, color="green", linestyle="--", alpha=0.5, label=f"GT loss={gt_loss:.4e}")
    ax.set_xlabel("Eval")
    ax.set_ylabel("Loss")
...
def run(args):
    ...
    print(f"  sigma0: {args.sigma0:.6e}")
    print(f"  tolfun(base): {args.tolfun:.6e}")
    print(f"  tolfunhist(base): {args.tolfunhist:.6e}")
        ...
        if trial.state == optuna.trial.TrialState.COMPLETE and trial.value is not None:
            if float(trial.value) < float(threshold):
                print(
                    ...
                    f"reached loss={float(trial.value):.6e} < {threshold:.6e}"
                )
                study.stop()
    ...
    for idx, npz_file in enumerate(npz_files):
        ...
        ir_peak_abs = float(np.max(np.abs(target_ir))) if target_ir.size > 0 else 0.0
        effective_tolfun = float(args.tolfun) * ir_peak_abs
        effective_tolfunhist = float(args.tolfunhist) * ir_peak_abs
        effective_early_stop_loss = float(args.early_stop_loss) * ir_peak_abs
        print(
            f"  IR peak(abs): {ir_peak_abs:.6e} | "
            f"effective_tolfun={effective_tolfun:.6e} | "
            f"effective_tolfunhist={effective_tolfunhist:.6e} | "
            f"effective_early_stop_loss={effective_early_stop_loss:.6e}"
        )
        ...
        def objective(trial):
            ...
            print(
                ...
                f"x0_norm_min={float(np.min(x0)):.3e} x0_norm_max={float(np.max(x0)):.3e}"
            )
            bp, bl, lh = run_cmaes(
                trial,
                target_ir,
        ...
        study.optimize(
            ...
            callbacks=[
                ...
                lambda study, trial: stop_on_success(study, trial, effective_early_stop_loss),
            ],
        )
        ...
        print(f"\n  Winner: Trial {best.number}, loss={best_loss:.6e} ({elapsed:.0f}s)")
        print(f"    Restarts run: {n_restarts_run}")
        for k in COMPOSITE_KEYS:
            print(f"    {k}: {comp[k]:.6e}")
        if gt_7:
            print(f"    NMSE: {compute_nmse_6d(best_params, gt_7):.6e}")
        ...
        row = {
            ...
            "loss_name": args.loss,
            "ir_peak_abs": float(ir_peak_abs),
            "effective_tolfun": float(effective_tolfun),
            "effective_tolfunhist": float(effective_tolfunhist),
            "effective_early_stop_loss": float(effective_early_stop_loss),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tolfun", type=float, default=5e-3, help="CMA-ES tolfun termination threshold")
