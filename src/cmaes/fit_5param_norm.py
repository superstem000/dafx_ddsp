"""CMA-ES + Optuna in normalized space for five-parameter plate fitting.

Stage-3 incremental ablation:
- optimize in normalized 5D space [-1, 1]
- synthesize with src.plate.SixParamPlate
- LHS restarts are sampled in 7D raw space, converted to six-parameter starts,
  then sampled mu is discarded and replaced with a fixed value.

Run as:
    python -m src.cmaes.fit_5param_norm --loss L1_STFT ...
"""
...
OPT5_KEYS = ["D_div_mu", "T0_div_mu", "Ly", "op_x", "op_y"]
OPT5_LO = np.array([SIX_BOUNDS[k][0] for k in OPT5_KEYS], dtype=np.float64)
OPT5_HI = np.array([SIX_BOUNDS[k][1] for k in OPT5_KEYS], dtype=np.float64)
OPT5_RANGE = OPT5_HI - OPT5_LO

NORM5_LO = -np.ones(len(OPT5_KEYS), dtype=np.float64)
NORM5_HI = np.ones(len(OPT5_KEYS), dtype=np.float64)
NORM5_RANGE = NORM5_HI - NORM5_LO
...
def compute_nmse_5d(est_6: dict, gt_6: dict) -> float:
    errs = []
    for key in OPT5_KEYS:
...
def norm5_to_physical5(norm5_batch_np: np.ndarray) -> np.ndarray:
    return OPT5_LO + ((norm5_batch_np - NORM5_LO) / NORM5_RANGE) * OPT5_RANGE


def physical5_to_norm5(phys5_batch_np: np.ndarray) -> np.ndarray:
    return NORM5_LO + ((phys5_batch_np - OPT5_LO) / OPT5_RANGE) * NORM5_RANGE
...
def generate_lhs_starts_norm5_from_raw7(n_trials: int, seed: int = 42, fixed_mu: float = 50.0) -> np.ndarray:
    ...
    six_phys[:, 0] = float(fixed_mu)
    opt5_phys = six_phys[:, 1:]
    opt5_norm = physical5_to_norm5(opt5_phys)
    return np.clip(opt5_norm, -1.0, 1.0)
...
def run_cmaes(
    ...
    if np.any(x0_norm5 < NORM5_LO) or np.any(x0_norm5 > NORM5_HI):
        raise ValueError(f"x0 is out of normalized bounds [-1, 1]. Got x0={x0_norm5}")
    ...
    opts = {
        "maxfevals": budget,
        "popsize": popsize,
        "bounds": [NORM5_LO.tolist(), NORM5_HI.tolist()],
        "seed": seed,
        "verb_disp": 0,
        "tolfun": 5e-3,
        "tolfunhist": 5e-3,
    ...
    es = cma.CMAEvolutionStrategy(x0_norm5.tolist(), float(sigma0), opts)
    eval_count = 0
    loss_history = []
    ...
    while not es.stop():
        ...
        opt5_phys = norm5_to_physical5(norm_solutions)
        six_phys = np.zeros((opt5_phys.shape[0], len(SIX_KEYS)), dtype=np.float64)
        six_phys[:, 0] = float(fixed_mu)
        six_phys[:, 1:] = opt5_phys
        plate_params = torch.tensor(six_phys, dtype=torch.float32, device=device)
    ...
    best_opt5 = norm5_to_physical5(best_norm)[0]
    best_p = {"mu": float(fixed_mu)}
    best_p.update({k: float(v) for k, v in zip(OPT5_KEYS, best_opt5)})
...
def plot_per_ir(stem, est_6, gt_6, loss_hist, best_loss, elapsed, out_path, gt_loss=None):
    ...
    if gt_6:
        ...
        ax.set_title(f"5 Opt Params (NMSE={compute_nmse_5d(est_6, gt_6):.4f})")
    else:
...
def plot_summary(results: list, out_path: Path):
    ...
    if results[0].get("gt_available", False):
        ...
        x = np.arange(len(OPT5_KEYS))
        axes[1].bar(
            x,
            [np.mean(errs[k]) for k in OPT5_KEYS],
            yerr=[np.std(errs[k]) for k in OPT5_KEYS],
            color="coral",
            capsize=4,
        )
        axes[1].set_xticks(x)
        axes[1].set_xticklabels(OPT5_KEYS, rotation=45)
...
def run(args):
    ...
    print(f"CMA-ES with LHS(raw7)->norm5 starts + SixParamPlate on {device.upper()}")
    print(f"  Loss: {args.loss}")
    print(f"  Available losses: {available}")
    print(f"  Trials: {args.n_trials}")
    print(f"  Duration: {args.duration}s")
    print("  Search: normalized 5D [-1, 1] space; expanded to six-parameter physical bounds")
    print("  Restarts: LHS in raw7, converted to six starts, sampled mu replaced with fixed mu")
    print(f"  fixed_mu: {args.fixed_mu}")
    ...
    lhs_starts_norm5 = generate_lhs_starts_norm5_from_raw7(
        args.n_trials,
        seed=args.lhs_seed,
    ...
    for idx, npz_file in enumerate(npz_files):
        ...
        study = optuna.create_study(
            study_name=f"norm5_lhs_{npz_file.stem}",
            storage=storage,
            load_if_exists=True,
            direction="minimize",
        ...
        def objective(trial):
            trial_idx = trial.number % len(lhs_starts_norm5)
            x0 = lhs_starts_norm5[trial_idx]
        ...
        if gt_6:
            print(f"    NMSE: {compute_nmse_5d(best_params, gt_6):.6f}")
        ...
        row = {
            ...
            "nmse": round(compute_nmse_5d(best_params, gt_6), 8) if gt_6 else None,
            "gt_available": gt_6 is not None,
            "fixed_mu": round(float(args.fixed_mu), 8),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output_dir", type=str, default="results/cmaes/incremental_ablation/3_5_norm")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--loss", type=str, default="L1_STFT")
    parser.add_argument("--storage", type=str, default="cmaes_lhs_5_norm.db")
    parser.add_argument("--budget", type=int, default=25000)
    parser.add_argument("--sigma0", type=float, default=0.6, help="CMA-ES initial step-size in normalized 5D space")
    parser.add_argument(
        "--fixed_mu",
        type=float,
        default=(MU_MIN + MU_MAX) / 2.0,
        help="Fixed mu value used in synthesis; sampled mu from restarts is discarded",
    )
