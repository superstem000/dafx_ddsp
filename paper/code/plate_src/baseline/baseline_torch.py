"""Torch-native baseline PSO for seven-parameter plate fitting.

Compared with src.baseline.baseline, this version keeps PSO particle state,
velocity updates, and objective evaluation in PyTorch tensors to reduce
NumPy<->Torch churn.

Run as:
    python -m src.baseline.baseline_torch
"""
...
def params_tensor_to_dict(params_row: torch.Tensor) -> dict:
    row = params_row.detach().cpu().numpy().tolist()
    return {k: float(v) for k, v in zip(PARAM_KEYS, row)}
...
def physical7_to_plate14_tensor(phys: torch.Tensor, device: str | torch.device) -> torch.Tensor:
    # phys: [B, 7] => [E, rho, h, Ly, T0, op_x, op_y]
    E = phys[:, 0]
    rho = phys[:, 1]
    h = phys[:, 2]
    Ly = phys[:, 3]
    ...
    B = phys.shape[0]
    const = lambda x: torch.full((B,), float(x), dtype=phys.dtype, device=device)

    out = torch.stack(
        [
            const(FIXED_PLATE_PARAMS["Lx"]),
            Ly,
            h,
            T0,
            rho,
            E,
            const(FIXED_PLATE_PARAMS["nu"]),
            const(FIXED_PLATE_PARAMS["T60_DC"]),
            const(FIXED_PLATE_PARAMS["T60_F1"]),
            const(FIXED_PLATE_PARAMS["loss_F1"]),
            const(FIXED_PLATE_PARAMS["fp_x"]),
            const(FIXED_PLATE_PARAMS["fp_y"]),
            op_x,
            op_y,
        ],
        dim=1,
...
def _evaluate_batch(
    x_norm_batch: torch.Tensor,
    target_t: torch.Tensor,
    duration: float,
    synth: BatchedModalPlateTorch,
    loss_fn,
    bounds_lo: torch.Tensor,
    bounds_range: torch.Tensor,
) -> torch.Tensor:
    phys = bounds_lo + x_norm_batch * bounds_range
    plate_params = physical7_to_plate14_tensor(phys, target_t.device)
    with torch.no_grad():
        audios = synth(plate_params, duration)
        losses = loss_fn(target_t.expand(phys.shape[0], -1), audios)
    return torch.nan_to_num(losses, nan=1e6, posinf=1e6, neginf=1e6)
...
def run_pso_torch(
    ...
    target_t = torch.tensor(target_ir, dtype=dtype, device=dev).unsqueeze(0)
    dim = len(PARAM_KEYS)

    bounds_lo = torch.tensor([PARAM_BOUNDS[k][0] for k in PARAM_KEYS], dtype=dtype, device=dev)
    bounds_hi = torch.tensor([PARAM_BOUNDS[k][1] for k in PARAM_KEYS], dtype=dtype, device=dev)
    bounds_range = bounds_hi - bounds_lo

    g = torch.Generator(device=dev)
    g.manual_seed(int(seed))

    particles = torch.rand((num_particles, dim), dtype=dtype, device=dev, generator=g)
    velocities = -0.1 + 0.2 * torch.rand((num_particles, dim), dtype=dtype, device=dev, generator=g)

    personal_best = particles.clone()
    personal_best_scores = _evaluate_batch(particles, target_t, duration, synth, loss_fn, bounds_lo, bounds_range)

    best_idx = int(torch.argmin(personal_best_scores).item())
    global_best = personal_best[best_idx].clone()
    global_best_score = float(personal_best_scores[best_idx].item())

    loss_history = personal_best_scores.detach().cpu().tolist()

    for _ in range(max_iter):
        r1 = torch.rand((num_particles, dim), dtype=dtype, device=dev, generator=g)
        r2 = torch.rand((num_particles, dim), dtype=dtype, device=dev, generator=g)

        velocities = w * velocities + c1 * r1 * (personal_best - particles) + c2 * r2 * (global_best.unsqueeze(0) - particles)
        particles = torch.clamp(particles + velocities, 0.0, 1.0)

        scores = _evaluate_batch(particles, target_t, duration, synth, loss_fn, bounds_lo, bounds_range)
        loss_history.extend(scores.detach().cpu().tolist())
        ...
        best_idx = int(torch.argmin(personal_best_scores).item())
        if float(personal_best_scores[best_idx].item()) < global_best_score:
            global_best = personal_best[best_idx].clone()
            global_best_score = float(personal_best_scores[best_idx].item())

    best_phys = bounds_lo + global_best * bounds_range
    return params_tensor_to_dict(best_phys), global_best_score, loss_history
...
def plot_per_ir(stem, est_7, gt_7, loss_hist, elapsed, out_path, gt_loss=None):
    ...
    ax.set_xlabel("Eval")
    ax.set_ylabel("Loss")
    ax.set_title("Convergence (PSO Torch)")
    ax.legend()
...
def run(args):
    ...
    print(f"Baseline Torch PSO + {args.loss} on {device.upper()}")
    ...
    for idx, npz_file in enumerate(npz_files):
        ...
        if gt_csv.exists():
            try:
                ...
                with torch.no_grad():
                    gt_tensor7 = torch.tensor([[gt_7[k] for k in PARAM_KEYS]], dtype=torch.float32, device=device)
                    gt_plate = physical7_to_plate14_tensor(gt_tensor7, device)
                    gt_audio = synth(gt_plate, args.duration)
                    gt_target = torch.tensor(target_ir, device=device, dtype=torch.float32).unsqueeze(0)
        ...
        t0 = time.time()
        best_params, best_loss, loss_hist = run_pso_torch(
            target_ir=target_ir,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output_dir", type=str, default="results/cmaes/baseline_pso_torch")
